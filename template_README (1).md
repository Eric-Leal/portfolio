...
Crie um arquivo **`.env`** na raiz da pasta `/frontend` e use o prefixo `VITE_` (ou `REACT_APP_` se estiver usando CRA) para expor as variáveis ao _bundle_ da aplicação.
...
> **Obs:** As variáveis de ambiente em projetos **Vite** precisam começar com `VITE_` para que o Vite as reconheça e as inclua no _bundle_ do frontend; variáveis sem esse prefixo não ficam disponíveis no código do cliente.
...
🚀 _O Back-end estará disponível em **http://localhost:8080**._
